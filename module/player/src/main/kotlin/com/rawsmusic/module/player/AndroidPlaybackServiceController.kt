...
internal class AndroidPlaybackServiceController(context: Context) {
    ...
    fun acquireUsbPlaybackWakeLock(reason: String) {
        runCatching {
            if (!usbPlaybackWakeLock.isHeld) {
                usbPlaybackWakeLock.acquire()
            }
        }.onSuccess {
            AppLogger.i(
                TAG,
                "USB playback wakelock ${if (usbPlaybackWakeLock.isHeld) "held" else "requested"}: " +
                    "reason=$reason timeout=none",
            )
        }.onFailure { error ->
