...
object AppPreferences {
    ...
    object Player {
        ...
        /**
         * User intent, independent from whether the native USB stream happened to be active
         * when the process was last persisted.
         */
        ...
        /** Stable shuffle traversal over the canonical queue. Songs themselves are never reordered. */
        var shuffleTraversalOrder: String
            get() = kv.decodeString("player_shuffle_traversal_order", "") ?: ""
            set(value) { kv.encode("player_shuffle_traversal_order", value) }

        var shuffleTraversalCursor: Int
        ...
        /** 普通 Android 输出使用播放器前置 PCM 增益接管用户音量。USB 独占不受影响。 */
        var androidDvcEnabled: Boolean
            get() = kv.decodeBool("player_android_dvc_enabled", false)
            set(value) { kv.encode("player_android_dvc_enabled", value) }

        var androidDvcVolume: Float
            get() = kv.decodeFloat("player_android_dvc_volume", volume).coerceIn(0f, 1f)
            set(value) { kv.encode("player_android_dvc_volume", value.coerceIn(0f, 1f)) }

        /** DVC 关闭时在普通 Android 输出的 DSP 链入口预留的峰值余量。 */
        var androidNoDvcHeadroomDb: Float
            get() = kv.decodeFloat("player_android_no_dvc_headroom_db", -6f).coerceIn(-24f, 0f)
            set(value) { kv.encode("player_android_no_dvc_headroom_db", value.coerceIn(-24f, 0f)) }
        ...
        var androidBinauralSpatialIntensity: Int
            get() = if (kv.containsKey("player_android_binaural_spatial_intensity")) {
                kv.decodeInt("player_android_binaural_spatial_intensity", 0).coerceIn(0, 100)
            } else if (androidBinauralSpatialEnabled) {
                55
            } else {
                0
            }
        ...
        /** PCM 降位抖动：0=关闭，1..9 对应 PcmDitherMode。默认 Modified E-weighted。 */
        var pcmDitherMode: Int
            get() = kv.decodeInt("player_pcm_dither_mode", 6).coerceIn(0, 9)
            set(value) { kv.encode("player_pcm_dither_mode", value.coerceIn(0, 9)) }

        /**
         * Internal DSP precision only. This does not change Android or USB
         * output encoding; bit-perfect playback remains a bypass path.
         */
        var internalDoublePrecisionProcessingEnabled: Boolean
            get() = kv.decodeBool("player_internal_double_precision", false)
            set(value) { kv.encode("player_internal_double_precision", value) }
        ...
        var usbHardwareVolume: Float
            get() = kv.decodeFloat("player_usb_hw_volume", 0.5f)
        ...
        var usbHardwareVolumeStep: Int
            get() = kv.decodeInt("player_usb_hw_volume_step", 30)
    ...
    object UI {
        ...
        var lastUpdateNotesVersionName: String
            get() = kv.decodeString("ui_update_notes_version_name", "") ?: ""
            set(value) { kv.encode("ui_update_notes_version_name", value) }
        ...
        /** 横屏播放器锁定：阻止传感器回到竖屏以及系统返回手势退出。 */
        var landscapePlayerLocked: Boolean
            get() = kv.decodeBool("ui_landscape_player_locked", false)
            set(value) { kv.encode("ui_landscape_player_locked", value) }
    ...
    object Scanner {
        ...
        /** 文件夹弹窗中本地路径与 SAF tree URI 的对应关系。 */
        private const val KEY_FOLDER_DIALOG_URI_BY_PATH = "scanner_folder_dialog_uri_by_path"

        var folderDialogUriByPath: Map<String, String>
            get() {
                val raw = kv.decodeString(KEY_FOLDER_DIALOG_URI_BY_PATH, "").orEmpty()
            ...
            set(value) {
                val normalized = value
                    .mapKeys { (path, _) -> path.replace('\\', '/').trim().trimEnd('/') }
                    .filterKeys { it.isNotBlank() }
                    .filterValues { it.isNotBlank() }
                kv.encode(KEY_FOLDER_DIALOG_URI_BY_PATH, gson.toJson(normalized))
            }
    ...
    object Equalizer {
        ...
        /** 互馈强度 (0.1%)。0 为旁路，1000 为完整设定衰减量。 */
        var crossfeedAmount: Int
            get() {
                val stored = kv.decodeInt("eq_crossfeed_amount", -1)
                if (stored >= 0) return stored.coerceIn(0, 1000)
                return if (crossfeedEnabled) 1000 else 0
            }
            set(value) {
                val safe = value.coerceIn(0, 1000)
                kv.encode("eq_crossfeed_amount", safe)
                kv.encode("eq_crossfeed_enabled", safe > 0)
            }
    ...
    object Lyrics {
        ...
        private fun migrateFlymeDefaultOff() {
            ...
            kv.encode(FLYME_DEFAULT_OFF_MIGRATION, true)
        }

        var tickerEnabled: Boolean
            get() {
                migrateFlymeDefaultOff()
                return kv.decodeBool("lyrics_ticker_enabled", false)
            }
        ...
        var desktopLyricEnabled: Boolean
            get() = kv.decodeBool("lyrics_desktop_enabled", false)
            set(value) { kv.encode("lyrics_desktop_enabled", value) }

        var desktopLyricLocked: Boolean
            get() = kv.decodeBool("lyrics_desktop_locked", false)
            set(value) { kv.encode("lyrics_desktop_locked", value) }

        var desktopLyricStatusBarMode: Boolean
            get() = kv.decodeBool("lyrics_desktop_status_bar_mode", false)
            set(value) { kv.encode("lyrics_desktop_status_bar_mode", value) }

        var desktopLyricStatusHideWhenPaused: Boolean
            get() = kv.decodeBool("lyrics_desktop_status_hide_paused", false)
            set(value) { kv.encode("lyrics_desktop_status_hide_paused", value) }

        var desktopLyricStatusHideInLandscape: Boolean
            get() = kv.decodeBool("lyrics_desktop_status_hide_landscape", false)
        ...
        var desktopLyricStatusPosition: Int
            get() = kv.decodeInt("lyrics_desktop_status_position", 1).coerceIn(0, 2)
            set(value) { kv.encode("lyrics_desktop_status_position", value.coerceIn(0, 2)) }

        var desktopLyricStatusWidthPercent: Int
            get() = kv.decodeInt("lyrics_desktop_status_width", 72).coerceIn(40, 100)
            set(value) { kv.encode("lyrics_desktop_status_width", value.coerceIn(40, 100)) }

        var desktopLyricStatusXOffset: Int
            get() = kv.decodeInt("lyrics_desktop_status_x_offset", 0).coerceIn(-640, 640)
            set(value) { kv.encode("lyrics_desktop_status_x_offset", value.coerceIn(-640, 640)) }

        /** 0 = left, 1 = center, 2 = right. */
        var desktopLyricStatusTextAlign: Int
            get() = kv.decodeInt("lyrics_desktop_status_text_align", 0).coerceIn(0, 2)
            set(value) { kv.encode("lyrics_desktop_status_text_align", value.coerceIn(0, 2)) }

        /** 0 = top, 1 = center, 2 = bottom. */
        var desktopLyricStatusVerticalAlign: Int
            get() = kv.decodeInt("lyrics_desktop_status_vertical_align", 0).coerceIn(0, 2)
            set(value) { kv.encode("lyrics_desktop_status_vertical_align", value.coerceIn(0, 2)) }

        /** 0 = off, 1 = translation, 2 = pronunciation. */
        var desktopLyricStatusSecondary: Int
            get() = kv.decodeInt("lyrics_desktop_status_secondary", 0).coerceIn(0, 2)
            set(value) { kv.encode("lyrics_desktop_status_secondary", value.coerceIn(0, 2)) }

        var desktopLyricStatusSecondaryOpacity: Int
            get() = kv.decodeInt("lyrics_desktop_status_secondary_opacity", 67).coerceIn(20, 100)
            set(value) { kv.encode("lyrics_desktop_status_secondary_opacity", value.coerceIn(20, 100)) }

        var desktopLyricStatusMergeSecondary: Boolean
            get() = kv.decodeBool("lyrics_desktop_status_merge_secondary", false)
            set(value) { kv.encode("lyrics_desktop_status_merge_secondary", value) }

        var desktopLyricStatusFontScale: Int
            get() = kv.decodeInt("lyrics_desktop_status_font_scale", 100).coerceIn(80, 220)
            set(value) { kv.encode("lyrics_desktop_status_font_scale", value.coerceIn(80, 220)) }

        var desktopLyricStatusSecondaryScale: Int
            get() = kv.decodeInt("lyrics_desktop_status_secondary_scale", 90).coerceIn(70, 180)
            set(value) { kv.encode("lyrics_desktop_status_secondary_scale", value.coerceIn(70, 180)) }

        var desktopLyricStatusOpacity: Int
            get() = kv.decodeInt("lyrics_desktop_status_opacity", 100).coerceIn(35, 100)
            set(value) { kv.encode("lyrics_desktop_status_opacity", value.coerceIn(35, 100)) }

        var desktopLyricStatusTextColor: Int
            get() = kv.decodeInt("lyrics_desktop_status_text_color", 0xFFFFFFFF.toInt())
            set(value) { kv.encode("lyrics_desktop_status_text_color", value) }

        var desktopLyricHideWhenPaused: Boolean
            get() = kv.decodeBool("lyrics_desktop_hide_paused", true)
            set(value) { kv.encode("lyrics_desktop_hide_paused", value) }

        var desktopLyricHideInLandscape: Boolean
            get() = kv.decodeBool("lyrics_desktop_hide_landscape", false)
            set(value) { kv.encode("lyrics_desktop_hide_landscape", value) }

        var desktopLyricShowTranslation: Boolean
            get() = kv.decodeBool("lyrics_desktop_translation", true)
            set(value) { kv.encode("lyrics_desktop_translation", value) }

        var desktopLyricShowRomanization: Boolean
            get() = kv.decodeBool("lyrics_desktop_romanization", false)
            set(value) { kv.encode("lyrics_desktop_romanization", value) }

        var desktopLyricShowBackground: Boolean
            get() = kv.decodeBool("lyrics_desktop_background", true)
            set(value) { kv.encode("lyrics_desktop_background", value) }

        var desktopLyricUseLyricFont: Boolean
            get() = kv.decodeBool("lyrics_desktop_use_lyric_font", true)
            set(value) { kv.encode("lyrics_desktop_use_lyric_font", value) }

        var desktopLyricWidthPercent: Int
            get() = kv.decodeInt("lyrics_desktop_width", 78).coerceIn(40, 100)
            set(value) { kv.encode("lyrics_desktop_width", value.coerceIn(40, 100)) }

        var desktopLyricFontScale: Int
            get() = kv.decodeInt("lyrics_desktop_font_scale", 100).coerceIn(80, 220)
            set(value) { kv.encode("lyrics_desktop_font_scale", value.coerceIn(80, 220)) }

        var desktopLyricSecondaryScale: Int
            get() = kv.decodeInt("lyrics_desktop_secondary_scale", 88).coerceIn(70, 180)
            set(value) { kv.encode("lyrics_desktop_secondary_scale", value.coerceIn(70, 180)) }

        var desktopLyricOpacity: Int
            get() = kv.decodeInt("lyrics_desktop_opacity", 100).coerceIn(35, 100)
            set(value) { kv.encode("lyrics_desktop_opacity", value.coerceIn(35, 100)) }

        var desktopLyricTextColor: Int
            get() = kv.decodeInt("lyrics_desktop_text_color", 0xFFFFFFFF.toInt())
            set(value) { kv.encode("lyrics_desktop_text_color", value) }

        var desktopLyricX: Int
            get() = kv.decodeInt("lyrics_desktop_x", Int.MIN_VALUE)
            set(value) { kv.encode("lyrics_desktop_x", value) }

        var desktopLyricY: Int
            get() = kv.decodeInt("lyrics_desktop_y", Int.MIN_VALUE)
            set(value) { kv.encode("lyrics_desktop_y", value) }
    }
    ...
    object LoudnessBalance {
        ...
        var loudnessPercent: Float
            get() = if (kv.containsKey("loudness_balance_amount")) {
                kv.decodeFloat("loudness_balance_amount", 0f).coerceIn(0f, 100f)
            } else if (isEnabled) {
                35f
            } else {
                0f
    ...
    object MonoBass {
        ...
        var amountPercent: Float
            get() = if (kv.containsKey("mono_bass_amount")) {
                kv.decodeFloat("mono_bass_amount", 0f).coerceIn(0f, 100f)
            } else if (isEnabled) {
                70f
            } else {
                0f
    ...
    object DynamicEq {
        ...
        var intensityPercent: Float
            get() = if (kv.containsKey("dynamic_eq_intensity")) {
                kv.decodeFloat("dynamic_eq_intensity", 0f).coerceIn(0f, 100f)
            } else if (isEnabled) {
                50f
            } else {
                0f
        ...
        var deEsserPercent: Float
            get() = if (kv.containsKey("dynamic_eq_deesser")) {
                kv.decodeFloat("dynamic_eq_deesser", 0f).coerceIn(0f, 100f)
            } else if (isEnabled) {
                45f
            } else {
                0f
    ...
    object MoogLadder {
        ...
        var mixPercent: Float
            get() = if (kv.containsKey("moog_ladder_mix")) {
                kv.decodeFloat("moog_ladder_mix", 0f).coerceIn(0f, 100f)
            } else if (isEnabled) {
                100f
            } else {
                0f
    ...
    object Surround360 {
        ...
        /** 效果强度 (0~100) */
        var intensity: Float
            get() = if (kv.containsKey("surround_360_intensity")) {
                kv.decodeFloat("surround_360_intensity", 0f).coerceIn(0f, 100f)
            } else if (isEnabled) {
                50f
            } else {
                0f
    ...
    object Panoramic360 {
        ...
        /** 效果强度 (0~100) */
        var intensity: Float
            get() = if (kv.containsKey("panoramic_360_intensity")) {
                kv.decodeFloat("panoramic_360_intensity", 0f).coerceIn(0f, 100f)
            } else if (isEnabled) {
                50f
            } else {
                0f
    ...
    object FftConvolver {
        ...
        var wet: Float
            get() = if (kv.containsKey("fft_convolver_wet")) {
                kv.decodeFloat("fft_convolver_wet", 0f).coerceIn(0f, 2f)
            } else if (isEnabled) {
                1f
            } else {
                0f
