...
fun BitmapImage(
    ...
    val sceneTransitionActive = LocalSharedTransitionSpec.current.active
    ...
    DisposableEffect(
        ...
        if (key.isBlank() || sceneTransitionActive) {
            onDispose { }
        } else {
            ...
            onDispose {
                // The mini player is switched as a complete content slot. Rapid previous/next
                // operations can dispose a slot and request the same song again immediately.
                // Keep an already-running decode alive so the next slot can join/cache it instead
                // of repeatedly cancelling the only artwork source and ending up empty.
                val keep = surface == ArtworkSurface.Playback ||
