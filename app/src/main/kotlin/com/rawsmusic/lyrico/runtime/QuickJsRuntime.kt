...
class QuickJsRuntime(
    ...
    override fun call(functionName: String, requestJson: String): String {
        val ptr = runtimePtr
        check(ptr != 0L) { "QuickJS runtime is closed" }
        val raw = QuickJsNative.call(ptr, functionName, requestJson)
        val parsed = runCatching { Json.parseToJsonElement(raw) }.getOrNull()
        return if (parsed is JsonPrimitive && parsed.isString) {
            parsed.contentOrNull.orEmpty()
        } else {
            raw
        }
    }
