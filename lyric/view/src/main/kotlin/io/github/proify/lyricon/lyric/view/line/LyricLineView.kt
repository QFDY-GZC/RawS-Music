...
open class LyricLineView(context: Context, attrs: AttributeSet? = null) :
    ...
    private var multiLineLayout: StaticLayout? = null
    ...
    fun setStyle(configs: LyricLineConfig) {
        ...
        syllable.isCharFloatAnimationEnabled = syllableConfig.enableCharFloatAnimation
        syllable.isSharpenEnabled = syllableConfig.enableSharpen
        syllable.sharpenIntensity = syllableConfig.sharpenIntensity
...
//    }
    ...
    override fun onMeasure(wSpec: Int, hSpec: Int) {
        ...
        if (w > 0 && lyric.text.isNotBlank()) {
            val textWidth = textPaint.measureText(lyric.text)
            if (textWidth > w) {
                val layout = StaticLayout.Builder.obtain(
                    lyric.text, 0, lyric.text.length, textPaint, w
                )
                ...
                setMeasuredDimension(w, resolveSize(layout.height, hSpec))
                return
            }
        }
    ...
    fun isMarqueeMode(): Boolean = false
    fun isOverflow(): Boolean = measuredWidth > 0 && lyricWidth > measuredWidth

    override fun onDraw(canvas: Canvas) {
        if (isMultiLineMode && multiLineLayout != null) {
            applyCurrentTextColor()
            multiLineLayout?.draw(canvas)
        } else if (isMarqueeMode()) marquee.draw(canvas) else syllable.draw(canvas)
    }
